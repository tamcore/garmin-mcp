...
| `mcp-stateless` | `true` | Serves the streamable-http transport without server-side sessions. The MCP SDK offers protocol `2026-07-28` (SEP-2575) **only** from a stateless transport, so turning this off makes the server advertise `2025-11-25` at best and a client that speaks only the newer protocol cannot negotiate a version at all. The cost is elicitation: a stateless transport cannot make a request of its client, so destructive tools refuse instead of prompting — their specified fail-closed behavior, but it does make the destructive tier unusable. Ignored by the stdio transport. |
| `log-tool-names` | `false` | Logs the exact tool name on every call. A tool name can itself disclose a medical domain, so it is off unless an operator wants per-call attribution. The pseudonymous principal, the category, the tier, the argument and result sizes, and a read-only call's own arguments are logged regardless. |
...
| `redirect-uris` | yes | 1 to 8 URIs, at most 2048 bytes each. Absolute, with a host, no userinfo, no fragment, no space and no control character. `https` always; `http` only for a loopback host — `127.0.0.1`, `::1` or the name `localhost`. The host must not be an IPv6 literal (`http://[::1]:port/...` or `https://[2001:db8::1]/...`): CSP3's `host-source` grammar has no production for a bracketed IPv6 literal, so a CSP-enforcing browser blocks the consent page's redirect to that client regardless of scheme; register a hostname, or an IPv4 loopback address such as `127.0.0.1`, instead. Matching at authorization time is byte-exact, with two exceptions. A registered **loopback** URI admits a presented one that differs only in the port, which RFC 8252 §7.3 requires because a native client takes an ephemeral port from the operating system at request time; scheme, host and path still match exactly, so `localhost` never matches `127.0.0.1`. And an entry may be a trailing-path wildcard registered under `oauth-allow-redirect-wildcards` below — without that setting, an entry carrying `*` is rejected outright. |
