...
| `redirect-uris` | yes | 1 to 8 exact URIs, at most 2048 bytes each. Absolute, with a host, no userinfo, no fragment, no `*`, no space and no control character. `https` always; `http` only for a **literal** loopback address. The name `localhost` is not accepted, because it resolves through a resolver an attacker may influence. Matching at authorization time is byte-exact. |
