...
| `v0.0.11` | published | `garmin_auth_status`, a live, safe probe of the Garmin session that reports whether a refresh is due without disclosing a token. Explicit stdio tier flags now authorize local writes, so an operator grants the write and destructive tiers the same way in both modes. Toolchain baseline raised to Go 1.27.0. |
| `v0.0.12` | published | Modern Go idioms applied across the tree: `for range` over integers, `slices` and `maps` in place of hand-written loops, `errors.Join`, and `omitzero` on the tool schemas. Behaviour unchanged. |
| `v0.0.13` | published | Opt-in redirect wildcards (`oauth-allow-redirect-wildcards`, default off) and the login email allowlist, recorded by ADR 0009. Eleven new domain reads — the raw FIT message view, course details and GPX, per-sport heart-rate zones, the calendar, the multi-night sleep range, the running-tolerance pair, acclimation and gear notes — and two decoding fixes: the VO2 max trend no longer drops carried-forward values, and the lactate threshold reports speed in metres a second. Plus the over-engineering sweep, which removed single-implementation seams, the placeholder keyring backend and unreachable plumbing. |
| `v0.0.14` | published | Streamable HTTP is served statelessly, which a current MCP client needs to connect at all. Per-call attribution logging: request ID, pseudonymous principal, client, outcome, latency, argument and result sizes, and the upstream endpoint label. A published Helm chart for a remote deployment. go-sdk to v1.8.0. |
| `v0.0.15` | published | Prometheus metrics on a listener of their own (`metrics-address`, empty and disabled by default): tool calls, upstream requests, login and refresh outcomes, registered-tool and build-info gauges. ADR 0010 records the dependency; the chart gained `metrics.enabled`, a `ServiceMonitor` and a `PrometheusRule`, all default off. |
| `v0.0.16` | published | A loopback redirect URI admits any port, which RFC 8252 §7.3 requires: a native MCP client takes an ephemeral port from the operating system at request time, so the port cannot be registered ahead of it. Scheme, host, path and query still match exactly. `localhost` is now loopback in `internal/oauthserver` too, agreeing with `internal/store` and `internal/config` — the disagreement let a `localhost` redirect URI register and then be refused at the authorize gate, surfacing as a generic 404 from `GET /login` with nothing logged. Reported as #1. |
