...
| `v0.0.10` | **uncommitted — not yet tagged.** HEAD is still `v0.0.9`; the row below describes work in the working tree, not a release | `garmin-mcp repair-permissions [--dry-run]`, the supported recovery from the v0.0.8/v0.0.9 ownership-and-mode check for a deployment a platform already widened before that check existed — the Kubernetes `fsGroup` recursive-remount case documented in `docs/operations.md`. It inspects the key directory and its key files, the active-key-version marker, the token directory and its records and lock files, every distinct configured OAuth client `secret-hash-file` (reported when it grants group or other access, never auto-fixed — see `docs/operations.md`), and, when `database-path` is set, the database, its `-wal`/`-shm` sidecars, and their parent directory; it tightens whatever this process's effective user already owns to exactly the mode `serve` requires (which can add an owner bit, not only narrow one) and reports exactly what changed; it refuses to touch anything owned by another local account or of the wrong object type, and exits non-zero whenever anything is left unresolved (including, under `--dry-run`, anything it would have tightened). A symlinked `--state-dir` ancestry is refused before anything is inspected. Inspection and fixing are two separate passes: a structural problem found anywhere blocks every fix in the same run, so a failing run never leaves partially-changed state, with one narrow, documented exception for the directory enumeration itself. Every check is a `stat`, every fix a `chmod` — the database is never opened as a database, so the command is safe beside a live `serve` process and in an init container before one starts. `internal/store` gained the small exported surface (`DatabaseFiles`, `DatabaseFileMode`, `SecureDirMode`, `TokenRecordFileMode`, `RecordsDirName`) this command needed to resolve the database's guarded paths without duplicating `OpenDatabase`'s own path resolution. |
