...
| `v0.0.8` | tagged from this commit | A secret file owned by another account no longer passes validation on its mode alone — the owner is compared to the effective uid, on the open descriptor. The SQLite database and its `-wal`/`-shm` sidecars are validated before the database is opened, rather than after it has been pinged. `LoadKey` verifies the key directory on every read. Deliberately absent: a hard-link check, an exclusive-rename install, a fallback, orphan recovery, a temporary sweeper and doctor parity — each introduced a state a deployment could not recover from, and each is recorded above. |
