...
`docs/adr/0001-garmin-login-feasibility.md`. The gate is now re-runnable: the
opt-in `garminlive` suite in `live/` re-performs the login, the DI exchange and
session validation against the real service. It landed as a tagged test suite
rather than as a command. When it later fails, re-enter the decision in
ADR 0001.
